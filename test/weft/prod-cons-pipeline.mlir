...
module {
  func.func @prod_cons_pipeline(%n : index) attributes { "num-threads" = 2 : i64 } {
    ...
    // Producer slots are mbarriers 0-2; consumer slots are mbarriers 3-5.
    // A single thread initializes them; the named-barrier sync (syncthreads)
    // orders the initialization before any thread's use.
    scf.if %is_tid0 {
      barrier.mbarrier_init 0, 1
      barrier.mbarrier_init 1, 1
      barrier.mbarrier_init 2, 1
      barrier.mbarrier_init 3, 1
      barrier.mbarrier_init 4, 1
      barrier.mbarrier_init 5, 1
    }
