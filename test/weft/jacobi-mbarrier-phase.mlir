...
module {
  func.func @jacobi_mbarrier_phase(%n : index) attributes { "num-threads" = 4 : i64 } {
    ...
    // this iteration's writes before the next iteration's reads. Note that
    // this implementation differs from the blog post, where it uses a single
    // set of addresses with barriers for forwards and anti-dependencies on the
    // data. The blog post instead uses separate buffers to avoid having to wait
    // on anti-dependencies.
    ...
    scf.for %i = %c0 to %n step %c1 iter_args(%phase = %false) -> (i1) {
      ...
      barrier.mbarrier_wait %read_gate %phase
      ...
      barrier.mbarrier_wait %write_gate %phase
